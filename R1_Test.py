...
def test_add_book_success_sets_available_and_success_message():
    success, message = add_book_to_catalog("R1 Book OK", "Author A", "1111111111111", 3)
    assert success == True
...
def test_add_book_rejects_non_digit_isbn():
    success, message = add_book_to_catalog("Bad ISBN chars", "Author", "ABCDEFGHIJKLM", 1)
    assert success == False
    assert "isbn" in message.lower() and "digit" in message.lower()
